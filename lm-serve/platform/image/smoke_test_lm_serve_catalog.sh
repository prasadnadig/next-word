#!/usr/bin/env bash
# Smoke-tests all routed models behind the Envoy edge service.
#
# The model list is discovered from route ConfigMap data generated by the
# reconciler, then each model receives a tiny OpenAI-style chat completion
# request using API key auth and/or JWT auth.
...
OPTIONS:
  ...
  --route-configmap NAME     Route ConfigMap (default: ${ROUTE_CONFIGMAP})
  --route-key KEY            Route data key (default: ${ROUTE_KEY})
...
fetch_routes_to_file() {
  ...
  local payload
  payload=$(kubectl ${kf} -n "${NAMESPACE}" get configmap "${ROUTE_CONFIGMAP}" -o "jsonpath={.data.${escaped_key}}" 2>/dev/null || true)

  [[ -n "${payload}" ]] || fatal "Could not read ${ROUTE_KEY} from ConfigMap ${ROUTE_CONFIGMAP}"
  printf '%s\n' "${payload}" > "${dst}"
}
...
main() {
  ...
  local route_file="${tmpdir}/routes.yaml"
  fetch_routes_to_file "${route_file}"
  ...
  local models
  models="$(yq -r '.routes[]?.prefix // ""' "${route_file}" | awk -F/ 'NF >= 5 && $4 != "" {print $4}' | sort -u)"
  [[ -n "${models}" ]] || fatal "No routed models found in ${ROUTE_CONFIGMAP}/${ROUTE_KEY}"
