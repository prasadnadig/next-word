...
```bash
helm template lm-serve-models models/helm -n lm-serve-dev-west \
  -f models/helm/values.yaml \
  -f models/helm/values.dev-west.yaml
```
...
helm upgrade --install lm-serve-publisher publisher/helm -n lm-serve-dev-west --create-namespace \
  -f publisher/helm/values.yaml
```
...
`publisher/helm/` and manage publisher rollout independently from model rollout.
