...
```bash
helm upgrade --install lm-serve-auth auth/helm -n lm-serve --create-namespace \
  -f auth/helm/values.yaml \
  --set-string authService.image.registry=localhost \
  --set-string authService.image.repository=lm-serve-auth-service \
  --set-string authService.image.tag=0.1.0

helm upgrade --install lm-serve-platform platform/helm -n lm-serve --create-namespace \
  -f platform/helm/values.yaml

helm upgrade --install lm-serve-models models/helm -n lm-serve --create-namespace \
  -f models/helm/values.yaml \
  -f models/helm/values.sample-env.yaml \
...
```bash
bash auth/image/generate_test_auth_materials.sh --team-size 3 --jwt-sub dev-user --jwt-ttl-sec 3600
```
...
make apply-serve-model \
  TENANT=sample-env \
  MODEL_RECONCILER_IMAGE_REGISTRY=<registry> MODEL_RECONCILER_IMAGE_REPOSITORY=vllm-catalog-deployer MODEL_RECONCILER_IMAGE_TAG=0.1.0
```
...
```bash
helm upgrade --install lm-serve-models models/helm -n lm-serve \
  -f models/helm/values.yaml \
  -f models/helm/values.sample-env.yaml \
  --set modelReconciler.enabled=true \
  --set-string modelReconciler.args.watchIntervalSec=60 \
  --set-string modelReconciler.image.registry=localhost \
  --set-string modelReconciler.image.repository=vllm-catalog-deployer \
  --set-string modelReconciler.image.tag=0.1.0
```
...
```bash
docker build -t localhost/vllm-catalog-deployer:0.1.0 models/image
docker push localhost/vllm-catalog-deployer:0.1.0
```
...
make build-model-reconciler-image MODEL_RECONCILER_IMAGE_REGISTRY=<registry> MODEL_RECONCILER_IMAGE_REPOSITORY=vllm-catalog-deployer MODEL_RECONCILER_IMAGE_TAG=0.1.0
make push-model-reconciler-image MODEL_RECONCILER_IMAGE_REGISTRY=<registry> MODEL_RECONCILER_IMAGE_REPOSITORY=vllm-catalog-deployer MODEL_RECONCILER_IMAGE_TAG=0.1.0
make apply-model-reconciler TENANT=sample-env MODEL_RECONCILER_IMAGE_REGISTRY=<registry> MODEL_RECONCILER_IMAGE_REPOSITORY=vllm-catalog-deployer MODEL_RECONCILER_IMAGE_TAG=0.1.0
```
...
```bash
bash platform/image/smoke_test_lm_serve_catalog.sh \
  --namespace lm-serve \
